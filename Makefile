...
pure_all :: $(INST_SCRIPT)/blog_test.pl $(INST_SCRIPT)/blog_server.pl $(INST_SCRIPT)/blog_fastcgi.pl $(INST_SCRIPT)/blog_create.pl $(INST_SCRIPT)/blog_cgi.pl
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) \
	  $(INST_SCRIPT)/blog_test.pl $(INST_SCRIPT)/blog_server.pl \
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <AUTHOR>Ehab Gamal,,,</AUTHOR>' >> $(DISTNAME).ppd
